#version 330 compatibility
#include "/lib/shadowDistort.glsl"
#define coloredGlassShadows
#define blacknwhite
#define inverted
#define graytorchlight

uniform sampler2D colortex0;
uniform sampler2D colortex1;
uniform sampler2D colortex2;
uniform sampler2D depthtex0;
uniform sampler2D shadowtex0;
uniform sampler2D shadowtex1;
uniform sampler2D shadowcolor0;
uniform int heldBlockLightValue;
uniform int heldBlockLightValue2;



/*
 const int colortex0Format = RGB16;
*/

uniform vec3 shadowLightPosition;
uniform mat4 gbufferModelViewInverse;
uniform mat4 gbufferProjectionInverse;
uniform mat4 shadowModelView;
uniform mat4 shadowProjection;

const vec3 blocklightColor = vec3(1.0);
const vec3 ambientColor = vec3(0.1);

in vec2 texcoord;

/* RENDERTARGETS: 0 */
layout(location = 0) out vec4 color;

vec3 projectAndDivide(mat4 projectionMatrix, vec3 position){
	vec4 homPos = projectionMatrix * vec4(position, 1.0);
	return homPos.xyz / homPos.w;
}

vec3 getShadow(vec3 shadowScreenPos){
   float transparentShadow = step(shadowScreenPos.z, texture(shadowtex0, shadowScreenPos.xy).r);

   if (transparentShadow == 1.0){
     return vec3(1.0);
   }

   float opaqueShadow = step(shadowScreenPos.z, texture(shadowtex1, shadowScreenPos.xy).r);
   if(opaqueShadow == 0.0){
     return vec3(0.0);
   }

   vec4 shadowColor = texture(shadowcolor0, shadowScreenPos.xy);

	//If colors are activated
	#ifdef coloredGlassShadows
   	return shadowColor.rgb * (1.5 - shadowColor.a);
	#endif
 }

void main() {

	vec2 lightmap = texture(colortex1, texcoord).xy;
	vec3 encodedNormal = texture(colortex2, texcoord).rgb;
	vec3 normal = normalize((encodedNormal - 0.5)*2.0);
	vec3 lightVector = normalize(shadowLightPosition);
  	vec3 worldLightVector = mat3(gbufferModelViewInverse) * lightVector;

	#ifdef inverted
	#ifdef blacknwhite


	vec4 originalColor = texture(colortex0, texcoord);
	float gray = dot(originalColor.rgb, vec3(0.2126, 0.7152, 0.0722));
	color = 1 - vec4(gray, gray, gray, originalColor.a);
	float depth = texture(depthtex0, texcoord).r;

	#else

	float depth = texture(depthtex0, texcoord).r;
	color = 1 - texture(colortex0, texcoord);
	color.rgb = pow(color.rgb, vec3(2.2));
	vec4 originalColor = texture(colortex0, texcoord);

	#endif
	#else


	#ifdef blacknwhite
	vec4 originalColor = texture(colortex0, texcoord);
	float gray = dot(originalColor.rgb, vec3(0.2126, 0.7152, 0.0722));
	color = vec4(gray, gray, gray, originalColor.a);
	float depth = texture(depthtex0, texcoord).r;

	#else

	float depth = texture(depthtex0, texcoord).r;
	color = texture(colortex0, texcoord);
	color.rgb = pow(color.rgb, vec3(2.2));
	vec4 originalColor = texture(colortex0, texcoord);

	#endif
	#endif

	if (depth == 1.0) {
    return;
	}
	vec3 ndcPos = vec3(texcoord.xy, depth) * 2.0 - 1.0;
  	vec3 viewPos = projectAndDivide(gbufferProjectionInverse, ndcPos); 
  	vec3 feetPlayerPos = (gbufferModelViewInverse * vec4(viewPos, 1.0)).xyz;
  	vec3 shadowViewPos = (shadowModelView * vec4(feetPlayerPos, 1.0)).xyz;
  	vec4 shadowClipPos = shadowProjection * vec4(shadowViewPos, 1.0);
  	shadowClipPos.z -= 0.001;
	const vec3 sunlightColor = vec3(1.0);
 	const vec3 skylightColor = vec3(0.1); //0.05, 0.15, 0.3
	shadowClipPos.xyz = distortShadowClipPos(shadowClipPos.xyz);
	vec3 shadowNdcPos = shadowClipPos.xyz / shadowClipPos.w;
  	vec3 shadowScreenPos = shadowNdcPos * 0.5 + 0.5;

	vec3 shadow = getShadow(shadowScreenPos);

   	vec3 blocklight = lightmap.x * blocklightColor;
   	vec3 skylight = lightmap.y * skylightColor;
   	vec3 ambient = ambientColor;
   	vec3 sunlight = sunlightColor * clamp(dot(worldLightVector, normal), 0.0, 1.0) * shadow;

	color.rgb *= blocklight + skylight + ambient + sunlight*0.5;


	float LightColorRight = smoothstep(heldBlockLightValue, 0.0, length(feetPlayerPos));
	float LightColorLeft = smoothstep(heldBlockLightValue2, 0.0, length(feetPlayerPos));
	
	#ifdef graytorchlight
	vec3 torchColor = vec3(0.7,0.7,0.7)*2;
	#else
	vec3 torchColor = vec3(1.0, 0.6, 0.3);
	#endif

	color.rgb += torchColor * LightColorRight/5.0; //??

}